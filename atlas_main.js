...
function resetAtlas(sceneIdx) {
  const idx = Math.max(0, Math.min(6, sceneIdx));
...
const SCENES = [
  ...
  { label: 'ZOOMING TO THE PLANE', sub: 'Camera approaching the right edge' },
  { label: 'SPOTTING 3I/ATLAS', sub: 'Moving left to find the interstellar visitor' },
...
function updateTimeline() {
  ...
  const dateMarkers = [
    ...
    { pos: sceneBounds[1], label: 'Zooming to Edge' },
    { pos: sceneBounds[2], label: 'JUL 1, 2025 · Discovery' },
    { pos: (sceneBounds[2] + sceneBounds[3]) / 2, label: 'SEP 2025 · Approaching' },
    { pos: sceneBounds[3], label: 'OCT 3, 2025 · Near Mars' },
    { pos: (sceneBounds[3] + sceneBounds[4]) / 2, label: 'OCT 29, 2025 · Perihelion' },
    { pos: sceneBounds[4], label: 'NOV 2025 · Outbound' },
    { pos: sceneBounds[5], label: 'DEC 19, 2025 · Earth Alignment' },
    { pos: (sceneBounds[5] + sceneBounds[6]) / 2, label: 'FEB 2026 · Heading to Jupiter' },
    { pos: sceneBounds[6], label: 'MAR 16, 2026 · Jupiter Flyby' }
  ];
...
function frame() {
  ...
  if (!isPaused) {
    ...
    // Special handling for Scene 0: auto-advance after 2 seconds
  ...
  if (!isPaused && atlasFrac < fracHi) atlasFrac = Math.min(atlasFrac + 0.00038, fracHi);
  const ap = atlasPos(atlasFrac);
  
  // Accumulate trail
  if (!isPaused) {
    atlasTrail.push({ wx: ap.wx, wy: ap.wy, wz: ap.wz, frac: atlasFrac });
    atlasFullTrail.push({ wx: ap.wx, wy: ap.wy, wz: ap.wz, frac: atlasFrac });
  }
  ...
  // ── Scene 1: Move to edge of solar plane (plane barely visible on right, NO 3I visible) ──
  else if (currentScene === 1) {
    const t1 = sceneTime;
    const progress = clamp(t1 / 180, 0, 1); // 3 seconds total
    
    // Camera moves far to the left edge, only seeing the plane edge on right
    const p1 = eio(progress);
    elTgt = 10;
    azTgt = lerp(1.57, 1.1, p1); // Look back towards plane from far left
    distTgt = lerp(1400, 600, p1);
    txTgt = lerp(0, -800, p1); // Move camera far to the left
    tyTgt = lerp(0, -500, p1);
    tzTgt = 0;
  }

  // ── Scene 2: Move left to spot 3I/ATLAS ──
  else if (currentScene === 2) {
    const t2 = sceneTime;
    const progress = clamp(t2 / 180, 0, 1); // 3 seconds
    const ap2 = atlasPos(atlasFrac);
    
    // Move camera slightly left to reveal 3I/ATLAS
    const p2 = eio(progress);
    elTgt = 10;
    azTgt = lerp(1.1, 1.5, p2);
    distTgt = 600;
    txTgt = lerp(-800, -600, p2); // Move slightly right to center 3I
    tyTgt = lerp(-500, ap2.wy * 0.6, p2);
    tzTgt = 0;
  }

  // ── Scene 3: Entering the system — 3I/ATLAS POV ──
  else if (currentScene === 3) {
    const ap2 = atlasPos(atlasFrac);
    elTgt = 5;
    azTgt = 5.24;
    const perihelionFrac = (atlasFrac - SC_FRAC[3][0]) / (SC_FRAC[3][1] - SC_FRAC[3][0]);
  ...
  // ── Scene 5: Dec 19 — Earth tail alignment ──
  ...
  for (let i = 0; i < N_PLANETS; i++) drawPlanet(planets[i], currentScene >= 4);
  drawSun();

  // Trail (show from Scene 2 onward when 3I appears)
  if (currentScene >= 2) {
    drawAtlasTrail();
  }

  // Comet (show from Scene 2 onward when 3I appears)
  if (currentScene >= 2) {
    drawComet(ap.wx, ap.wy, ap.wz, 1, '120,255,200');
  }
